...
export function TabsList({ className, children, ...props }: RadixTabs.TabsListProps) {
  ...
  return (
    <RadixTabs.List
      ref={containerRef}
      className={cn(
        "relative inline-flex items-center gap-1 rounded-md bg-primary-50 p-1 md:rounded-full",
        className,
      )}
      {...props}
    >
...
export function TabsTrigger({ className, ...props }: RadixTabs.TabsTriggerProps) {
  return (
    <RadixTabs.Trigger
      className={cn(
        "relative z-10 flex h-14 w-16 flex-col items-center justify-center gap-1 rounded-md px-1 text-[11px] font-medium text-neutral-600 md:h-11 md:w-auto md:flex-row md:gap-1.5 md:rounded-full md:px-4 md:text-sm",
